...
This repository adopts Eric's Engineering Constitution through the pinned `constitution/` submodule. The current branch pins the submodule to Constitution `1.39.1` on `main` and includes local guardrails for Constitution version checks, compliance checks, declared-test runs, documentation-freshness checks, secrets sweeps, OTS inventory cross-checks, environment-contract checks, architecture checks, Dependabot submodule updates, pre-commit, Aider, Continue, Goose, Solon, and devcontainers.
