...
| OTS-002 | `keyring` | Declared `>=23.0.0` in `pyproject.toml` | Python packaging authority / Jason R. Coombs, MIT | Accessing OS native credential stores (Keychain, KWallet, Secret Service, Credential Manager) for secure SSH password storage | Medium | Exercised via unit tests in `TestKeyringIntegration` (`test_SSH_DeviceManager.py`) covering saving, retrieving, deleting, and fallback logic when OS keyring is missing or unavailable | [GitHub advisories](https://github.com/jaraco/keyring/security/advisories) — last reviewed 2026-08-27 | Open ceiling so pip resolves the newest compatible release | Active |
