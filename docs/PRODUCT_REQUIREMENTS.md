...
| **FR-009** | SHOULD | Support storing and retrieving SSH passwords via OS keyring when opted in. | FR-009-AC-1 saves passwords securely in OS keyring instead of plain profile files. FR-009-AC-2 loads stored keyring passwords automatically when profile/host is selected. FR-009-AC-3 degrades gracefully if keyring service is missing. |
...
| **NFR-002** | MUST | Treat SSH credentials and trust decisions as security-sensitive. | NFR-002-AC-1 passwords are not persisted in profile files. NFR-002-AC-2 unknown host-key handling is explicit and configurable. NFR-002-AC-3 credentials can be cleared on disconnect. |
...
- This project does not manage SSH private keys yet.
