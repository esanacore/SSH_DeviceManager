...
| FR-009 | SHOULD | Store and retrieve passwords via OS keyring securely when opted in. | `TestKeyringIntegration` | Verified |
...
| NFR-003 | SHOULD | Run deterministic tests without real SSH devices. | Full suite: `python3 -m unittest test_SSH_DeviceManager.py test_customizer.py -v` | Verified |
...
| Status | Count |
| --- | ---: |
| Requirements with verifying tests | 13 |
