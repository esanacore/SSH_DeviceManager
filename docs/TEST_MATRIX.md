...
3. [Integration Tests](#integration-tests)
   - [Full Lifecycle](#12-connect--run--disconnect-lifecycle)
   - [Auth Failure Recovery](#13-auth-failure-recovery)
   - [Validation Blocks Connect](#14-validation-blocks-connect)
   - [Profile Workflow](#15-profile-workflow)
   - [Sections JSON Loading](#16-sections-json-loading)
   - [Command History](#17-command-history-integration)
   - [Command Failure Handling](#18-command-failure-handling)
   - [Config Round-Trip](#19-app-config-round-trip)
   - [Host History Limit](#20-host-history-limit)
   - [Disconnect Credential Clearing](#21-disconnect-credential-clearing)
...
| GUI App Core | `TestSSHGuiApp` | 9 | Unit |
| Input Validation | `TestGetConnectionInputs` | 11 | Unit |
...
| UT-VAL-11 | (covered by `test_valid_inputs`) | Boundary: port=22, timeout=10 | Accepted as valid |
...
### 12. Connect -> Run -> Disconnect Lifecycle
...
### 17. Command History Integration

| ID | Test Method | Flow |
|---|---|---|
| IT-CH-01 | `test_history_ordering_and_dedup` | Run cmd_a, cmd_b, cmd_c -> History = [cmd_c, cmd_b, cmd_a] -> Run cmd_a again -> History[0] = cmd_a (moved to front) |
| IT-CH-02 | `test_history_limit` | Run COMMAND_HISTORY_LIMIT + 50 unique commands -> History length <= COMMAND_HISTORY_LIMIT |

### 18. Command Failure Handling
...
### 21. Disconnect Credential Clearing
...
| REQ-CONN-03 | Connection state detected via transport check | UT-SSH-01..04 |
| REQ-CONN-04 | Concurrent connect attempts blocked | UT-APP-02 (is_connecting guard) |
...
| REQ-CMD-02 | Command output displayed (including empty output) | UT-SSH-09 |
...
| REQ-CMD-05 | Command without connection shows warning | UT-SSH-08 |
| REQ-HIST-01 | Commands added to history, most recent first | IT-CH-01 |
| REQ-HIST-02 | Duplicate commands moved to front | IT-CH-01 |
...
| REQ-PROF-01 | Profiles saved with connection details | UT-PR-01, IT-PW-01 |
| REQ-PROF-02 | Profiles loaded and fields populated | UT-PR-03, IT-PW-01 |
| REQ-PROF-03 | Missing profile handled gracefully | UT-PR-04 |
